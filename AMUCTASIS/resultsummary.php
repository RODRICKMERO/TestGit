     ...
     <table width="289" height="36" border="0">
       <tr>
         <th width="283" height="32" scope="col"> <form id="form1" name="form1" method="get"  action="">
           <table width="366" height="44" border="0">
             <tr>
               <td width="111" height="40"><span class="style53">Select Year</span></td>
              ...
              ?>
               ...
               <td width="64"><input name="Save" type="submit" id="Save" value="OK" /></td>
             </tr>
           </table>
...
$page_name="resultsummary.php"; //  If you use this code with a different page ( or file ) name then change this 
$start=$_GET['start'];
...
echo " RESULT SUMMARY ".strtoupper($faculty)." FIRST YEAR STUDENTS  ";
}
...
echo " RESULT SUMMARY ".strtoupper($faculty)." SECOND YEAR STUDENTS  ";
}
if ($maxmam==3){



echo "RESULT SUMMARY ".strtoupper($faculty)."  THIRD YEAR STUDENTS  ";
}
...
echo" RESULT SUMMARY ".strtoupper($faculty)."  FORTH YEAR STUDENTS  ";
}
if ($maxmam==5){



echo " RESULT SUMMARY ".strtoupper($faculty)."  FIFTH YEAR STUDENTS  ";
}
}
echo"</center>";
$maxmam=0;
if ($v==0){
	...
	$qux="select * from courses where ((courses.faculty='$faculty')and (courses.course_year='$year' ));"; 
$rex=mysql_query($qux);
...
echo"<th align='left'>";echo"GPA";echo"</th>";
echo"</tr>";
$v++;
}
		   $hh++;
		   $sio=0;
		     $TOTALPOINT=0; $TOTALUNIT=0; $GPA=0;$POINT=0;
		   	 for ($k=1;$k<=$f;$k++) {
			 set_time_limit(0);	 
...
if(!empty($kapu['score'])){
$POINT=($UN * ("".$kapu['course_unit'].""));
$TOTALPOINT=$TOTALPOINT+$POINT; $TOTALUNIT=$TOTALUNIT+("".$kapu['course_unit']."");
}
		   if($a==1)
			{ 
...
if(!empty($kapu['score'])){echo"".$kapu['marks']."";
}else { echo".";}echo"</td>" ;
			  $a="2"; 			  
			 }
           else
