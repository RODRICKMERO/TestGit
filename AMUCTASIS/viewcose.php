     ...
     <table width="408" border="0" cellspacing="0" cellpadding="0">
       <tr>
         <td width="425" height="29"><strong>
           <marquee>
             <span class="style35">VIEW COURSE</span>
             </marquee>
         </strong></td>
       </tr>
     </table>
...
$page_name="viewcose.php"; //  If you use this code with a different page ( or file ) name then change this 
$start=$_GET['start'];
...
$a="1";
$query="select  * from courses,upload_course where (courses.faculty=(select faculty from staff_faculty where (staff_faculty.id_no='".$_SESSION['USERREG']."')) and (courses.course_code=upload_course.course_code))order by course_year;";
$result=mysql_query($query);
$nume=mysql_num_rows($result);

echo '<table align="center" width="" border="1" cellpadding="1" cellspacing="0" bordercolor="#66cccc">';
echo "<tr bgcolor='#6699CC'><th>Sn</th><th align='left'>C_CODE:</th><th align='left'> NAME:</th><th align='left'> UNIT:</th><th align='left'> FACULTY:</th><th align='left'> YEAR:</th><th align='left'>SEM.:</th><th align='left'>COM.:</th><th align='left'>INFO</th> </tr>";
 
$query2="select  * from courses,upload_course where (courses.faculty=(select faculty from staff_faculty where (staff_faculty.id_no='".$_SESSION['USERREG']."')) and (courses.course_code=upload_course.course_code))order by course_year, semester limit $eu, $limit ;"; 
...
echo "<tr bgcolor='#ffdfaa'><td>$n</td><td>".strtoupper("".$laswai['course_code']."")."</td><td>".$laswai['course_name']."</td><td> ".$laswai['course_unit']."</td><td> ".$laswai['faculty']."</td><td>".$laswai['course_year']."</td><td> ".strtoupper("".$laswai['semester']."")."</td><td>".ucfirst("".$laswai['comment']."")."</td><td><a href='download.php?id=".$laswai['course_code']."'>".$laswai['cname']."</a></td></tr> " ;
			  $a="2"; 			  
			 }
           else
              {
			   
echo "<tr bgcolor='#EEEEEE'><td>$n</td><td>".strtoupper("".$laswai['course_code']."")."</td><td>".$laswai['course_name']."</td><td> ".$laswai['course_unit']."</td><td> ".$laswai['faculty']."</td><td>".$laswai['course_year']."</td><td> ".strtoupper("".$laswai['semester']."")."</td><td>".ucfirst("".$laswai['comment']."")."</td><td><a href='download.php?id=".$laswai['course_code']."'>".$laswai['cname']."</a></td></tr> " ;
			  $a="2"; 			  	
				 $a="1";
