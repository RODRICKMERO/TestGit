...
$page_name="viewuser2.php"; //  If you use this code with a different page ( or file ) name then change this 
$start=$_GET['start'];
...
$query=" select * from master_user2  where not(master_user2.group  ='student' )order by id_no;";
...
?>
               <div id="printReady">
               <?php
echo '<table align="center" width="600" border="1" cellpadding="1" cellspacing="0" bordercolor="#000000">';

?>


<form name=form1 method='post' action='' >
<tr bgcolor='#6699CC'><th align='left'>USERNAME</th><th align='left'>ID_NO</th><th align='left'>GROUP</th><th  width='10%'>DEL</tr>

<?php    
$query2=" SELECT * FROM master_user2 where not(master_user2.group  ='student') order by id_no limit $eu, $limit ";
     ...
     while( $laswai=mysql_fetch_array($result2) )
	        {
		     if($a==1)
			 {
			   ...
			   echo "<tr bgcolor='#66CCCC'><td>".ucwords("".$laswai['name']."")."</td><td>".$laswai['id_no']."</td><td> ".$laswai['group']."</td><td align=center >";
			  ?>
			   <input name=checkbox[] type=checkbox id=checkbox[]  class="case"  value="<?php echo $laswai['id_no'];?>"></td></tr>
           ...
           else
              {
			   ...
			   echo "<tr bgcolor='#EEEEEE'><td>".ucwords("".$laswai['name']."")."</td><td>".$laswai['id_no']."</td><td> ".$laswai['group']."</td>";
...
for($i=0;$i<$nume;$i++){
$del_id = $checkbox[$i];
$sql = "DELETE FROM master_user2 WHERE id_no ='$del_id'";
$result = mysql_query($sql);
$sql1 = "DELETE FROM staff_faculty WHERE id_no ='$del_id'";
...
if($result){
echo "<meta http-equiv=\"refresh\" content=\"0;URL=viewuser2.php\">";
}
}
mysql_close();
...
?>
       ...
       <a href="javascript:void(printSpecial())"><img src="images/b_print.png" border="0" height="20" width="20" alt="[print]" /></a><p><hr width='70%' />
<?php
...
//-->
     ...
     <div id="wb_MenuBar1" style="position:absolute; left:159px; top:121px; width:92px; height:22px; z-index:1001;">
       <div id="MenuBar1">
