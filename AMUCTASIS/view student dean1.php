...
$page_name="viewstudentdean.php"; //  If you use this code with a different page ( or file ) name then change this 
$start=$_GET['start'];
...
echo mysql_error();
  ...
  $v=0;$tp=0;
    while( $laswai=mysql_fetch_array($result2) )
			...
			$q="select * from courses,course_regsration where ((course_regsration.id_no='".$laswai['id_no']."')and (course_regsration.course_code=courses.course_code)and (courses.faculty='$faculty')) order by course_year ";
$re=mysql_query($q);
...
$querya="select * from perfomance,courses where ((perfomance.id_no='".$laswai['id_no']."') and (perfomance.course_code=courses.course_code)and(courses.faculty='$faculty')) order by course_year,semester ;"; 
