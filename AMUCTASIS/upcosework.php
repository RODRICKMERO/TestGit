...
<table width="408" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="425" height="29"><strong>
      <marquee>
        <span class="style35">Upload Coursework</span>
      </marquee>
      </strong></td>
    </tr>
</table>
...
$quer= "INSERT INTO systemtracer values('','".date("20y-m-d")."',' COSEWORK  WAS ADED IN THE SYSTEM BY ".$ngonyan['name']." AND ".$_SESSION['USERREG']." REGSRATION NUMBER IN ".date("20y-m-d")." at ".time("h:m:s")."')";
$resul=mysql_query($quer);

$quer= "INSERT INTO systemtracer2 values('','".date("20y-m-d")."',' COSEWORK  WAS ADED IN THE SYSTEM BY ".$ngonyan['name']." AND ".$_SESSION['USERREG']." REGSRATION NUMBER IN ".date("20y-m-d")." at ".time("h:m:s")."')";
$resul=mysql_query($quer);
mysql_error;
...
$sqlt="select * from course_regsration where course_regsration.course_code='$coursecode'and course_regsration.id_no='$idno' and course_regsration.y_study='$year'";
$re=mysql_query($sqlt);
while($mmm=mysql_fetch_array($re)){

$aaa="select * from staffcourse where staffcourse.id_no='".$_SESSION['USERREG']."' and staffcourse.course_code='$coursecode' and staffcourse.year='".$mmm['y_study']."'";
$aa=mysql_query($aaa);
$aaaa=mysql_num_rows($aa);

if ($aaaa>=1){

$sql = "insert into coursework set
id_no='$idno',
...
$_POST='';
 ...
 /*echo "<script language='javascript'>
                alert('$msg; $aaaa');
				window.location='upcosework.php';
                </script>";*/
