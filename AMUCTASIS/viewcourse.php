     ...
     <table width="408" border="0" cellspacing="0" cellpadding="0">
       <tr>
         <td width="425" height="29"><strong>
           <marquee>
             <span class="style35">DELETE COURSE </span>
             </marquee>
         </strong></td>
       </tr>
     </table>
...
$page_name="viewcourse.php"; //  If you use this code with a different page ( or file ) name then change this 
$start=$_GET['start'];
...
$a="1";
$query="select  * from courses where courses.faculty=(select faculty from staff_faculty where (staff_faculty.id_no='".$_SESSION['USERREG']."'))order by course_year;";
$result=mysql_query($query);
$nume=mysql_num_rows($result);
echo"<form name=form1 method='post' action='' >";


echo '<table align="center" width="90%" border="1" cellpadding="0" cellspacing="0" bordercolor="#66cccc" >';
echo "<tr bgcolor='#6699CC'><th>Sn</th><th align='left'>COURSE_CODE:</th><th align='left'> NAME:</th><th align='left'> UNIT:</th><th align='left'> FACULTY:</th><th align='left'> YEAR:</th><th align='left'>SEMESTER:</th><th align='left'>COMMENT:</th><th align='left'>SELECT:</th> </tr>";
 
$query2="select  * from courses where courses.faculty=(select faculty from staff_faculty where (staff_faculty.id_no='".$_SESSION['USERREG']."'))order by course_year, semester limit $eu, $limit ;"; 
...
echo "<tr bgcolor='#ffdfaa'><td>$x</td><td>".strtoupper("".$laswai['course_code']."")."</td><td>".ucfirst("".$laswai['course_name']."")." </td><td> ".$laswai['course_unit']."</td><td> ".$laswai['faculty']."</td><td>".$laswai['course_year']."</td><td>".strtoupper("".$laswai['semester']."")."</td><td> ".$laswai['comment']."</td><td>";?>
...
echo "<tr bgcolor='#EEEEEE'><td>$x</td><td>".strtoupper("".$laswai['course_code']."")."</td><td>".ucfirst("".$laswai['course_name']."")."</td><td> ".$laswai['course_unit']."</td><td> ".$laswai['faculty']."</td><td>".$laswai['course_year']."</td><td> ".strtoupper("".$laswai['semester']."")."</td><td> ".$laswai['comment']."</td><td>";?>
     <input name="checkbox[]" type="checkbox" id="checkbox[]"  class="case"  value="<?php echo $laswai['course_code'];?>" />
     <?php echo"</td></tr>";?>
     <?php
			  $a="2"; 			  	
...
for($i=0;$i<$nume;$i++){
$del_id = $checkbox[$i];
$query3="DELETE FROM courses WHERE (courses.course_code='$del_id')and courses.faculty=(select faculty from staff_faculty where (staff_faculty.id_no='".$_SESSION['USERREG']."'));";
	$result3=mysql_query($query3);
	//$query4="DELETE FROM upload_course WHERE (upload_course.course_code='$del_id');";
