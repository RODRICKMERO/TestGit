     ...
     <table width="408" border="0" cellspacing="0" cellpadding="0">
       <tr>
         <td width="425" height="29"><strong>
           <marquee>
             <span class="style35">COURSEWORK RESULTS </span>
             </marquee>
         </strong></td>
       </tr>
     </table>
...
$page_name="coseworkedit.php"; //  If you use this code with a different page ( or file ) name then change this 
$start=$_GET['start'];
...
$eu = ($start - 0); 
$limit = 10000000;                                 // No of records to be shown per page.
$this1 = $eu + $limit; 
$back = $eu - $limit; 
$next = $eu + $limit; 
...
$query41="select*FROM coursework";
	$result41=mysql_query($query41);
$num=mysql_num_rows($result41);

$snn=0;
$query="select  * from student,coursework where ((coursework.year='$y_study') and (coursework.course_code='$course') and (student.id_no=coursework.id_no));";
$result=mysql_query($query);
$nume=mysql_num_rows($result);

echo ' <font color="blue"><center>';echo"This is $course Coursework Results  in  $y_study";echo ' </font></center>';

echo"<form name=form1 method='post' action='' >";
echo '<table align="center" width="" border="1" cellpadding="2" cellspacing="0" bordercolor="#66cccc">';
echo "<tr bgcolor='#6699CC'><th align='left'>SN:</th><th align='left'>STUDENT NAME:</th><th align='left'>REG_NO:</th><th align='left'>MARKS:</th><th align='left'>Del:</th></tr>";
 $query2="select  * from student,coursework where ((coursework.year='$y_study') and (coursework.course_code='$course') and (student.id_no=coursework.id_no))limit $eu, $limit "; 
...
echo mysql_error();
    ...
    while( $laswai=mysql_fetch_array($result2) )
			...
			$x="".$laswai['id_no'].""."".$course."";
			//echo"$x";
		     if($a==1)
			 {
			   
echo "<tr bgcolor='#ffdfaa'><td>$snn</td><td>".ucwords("".$laswai['name']."")."</td><td> ".$laswai['id_no']."</td><td> ".$laswai['marks']."</td><td>";?>
...
echo "<tr bgcolor='#eeeeee'><td>$snn</td><td>".ucwords("".$laswai['name']."")."</td><td> ".$laswai['id_no']."</td><td> ".$laswai['marks']."</td><td>";?>
     <input name="checkbox[]" type="checkbox" id="checkbox[]"  class="case"  value="<?php echo $x;?>" />
     <?php echo"</td></tr>";
			  $a="2"; 			  	
				 $a="1";
...
$query3="DELETE FROM coursework WHERE ((coursework.id_no='".$sq['id_no']."')and(coursework.course_code='".$sq['course_code']."'));";
...
/*$quer= "INSERT INTO systemtracer values('','".date("20y-m-d")."',' COURSEWORK  $del_id WAS DELETED FROM THE SYSTEM BY THE DEAN OF FACULTY OF ".$ngonyan['faculty']."  WITH THE NAME ".$ngonyan['name']." AND ".$_SESSION['USERREG']." REGSRATION NUMBER IN ".date("20y-m-d")." at ".time("h:m:s")."')";
...
echo "<script language='javascript'>
                alert('coursework $n was deleted successfully');
				                </script>";
